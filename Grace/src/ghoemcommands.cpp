...
// IPMI OEM DIAG Major and Minor version
#ifdef CPU_DIAG_ENABLE
static constexpr uint8_t OEM_DIAG_MAJOR_VER = 0x01;
static constexpr uint8_t OEM_DIAG_MINOR_VER = 0x00;
static constexpr uint8_t OEM_DIAG_PATCH_VER = 0x00;
    ...
    if ((request % 10 == 0) && (request <= 100) && (request >= 20))
    {
        ...
        if (!ofs.is_open())
        {
            std::string errString =
                "ipmiSetFanZonePWMDuty: Failed to open hwmon pwm file. zone = " +
...
{
    auto method = dbus->new_method_call(systemBusName, systemPath, systemIntf,
                                        action.c_str());
...
ipmi::RspType<uint8_t> ipmiGetipmiChannelRfHi()
{
    ...
    {
        log<level::ERR>("Corrupted channel config.", entry("MSG=%s", e.what()));
        return ipmi::responseResponseError();
    }
                            ...
                            uint8_t disableCredBootStrap)
{
    ...
    try
    {
        ...
        dbus->async_send(method,
                         [dbus, service, userName, password,
                          disableCredBootStrap](boost::system::error_code ec,
...
bool isServiceActive(const std::string& serviceName)
{
    ...
    auto bus = getSdBus();
    auto message =
        bus->new_method_call("org.freedesktop.systemd1", objectPath.c_str(),
    ...
    {
        log<level::ERR>("Failed to get the service state",
                        phosphor::logging::entry("EXCEPTION=%s", e.what()));
        return false;
    }
}
...
ipmi::RspType<uint8_t> ipmicmdStandByPowerOnOff(uint8_t StandByPowerOption)
{
    ...
    {
        // To start nvidia-standby-poweroff 'run power' has to be DOWN i.e
        // RUN_POWER_EN = 0
        std::string standbyPoweroff =
            "systemctl start nvidia-standby-poweroff.service";
    ...
    {
        // To start nvidia-standby-poweron 'run power' should be UP which will
        // turn nvidia-standby-poweroff down and set RUN_POWER_PG = 1
        std::string standbyPoweron =
            "systemctl start nvidia-standby-poweron.service";
    ...
    else if (StandByPowerOption == 0x02)
    {
        ...
        bool isStandbyPowerOffServiceActive =
            isServiceActive("nvidia_2dstandby_2dpoweroff_2eservice");
        bool isStandbyPowerOnServiceActive =
            isServiceActive("nvidia_2dstandby_2dpoweron_2eservice");
...
ipmi::RspType<uint8_t> ipmiStandbyPowerCycle()
{
    ...
    phosphor::logging::log<level::INFO>("Starting standby power cycle");
    ...
    setDiag(ipmi::Context::ptr ctx,uint8_t mode)
{
    ...
    stopDiagTimerString += nvidia::diagServiceList;
    ...
    try
    {
        ...
        if(mode == 0)
        {
            ...
            {
                phosphor::logging::log<level::ERR>(
                    "ipmiDiagFlowCtrl: service failed to stop");
                return ipmi::responseResponseError();
            }
        }
        else if(mode == 1)
        {
            ...
            {
                phosphor::logging::log<level::ERR>(
                    "ipmiDiagFlowCtrl: service failed to start");
                return ipmi::responseResponseError();
            }
        }
        else
    ...
    try
    {
        ...
        for(auto& item : j)
        {
            if(item["Tid"] == tid)
            {
                ...
                item["DynamicDataSize"] = dynamicDataSize;
                item["DynamicData"] = dynamicData;
                found = true;
                break;
            }
    ...
    try
    {
        ...
        for (auto& item : j)
        {
            if(item["Tid"] == tid)
            {
                ...
                value = item["ResultMask"].get<std::vector<uint8_t>>();
                found = true;
                break;
            }
        }
...
void registerNvOemFunctions()
{
    ...
    ipmi::registerHandler(ipmi::prioOemBase, ipmi::nvidia::netFnOemNV,
                          ...
                          ipmi::Privilege::Admin, ipmi::getDiag);

    log<level::NOTICE>(
        "Registering ", entry("NetFn:[%02Xh], ", ipmi::nvidia::netFnOemNV),
        entry("Cmd:[%02Xh]", ipmi::nvidia::misc::cmdSetDiagSystemConfig));

    ipmi::registerHandler(ipmi::prioOemBase, ipmi::nvidia::netFnOemNV,
                          ipmi::nvidia::misc::cmdSetDiagSystemConfig,
                          ipmi::Privilege::Admin, ipmi::setDiagSystemConfig);

    log<level::NOTICE>(
        "Registering ", entry("NetFn:[%02Xh], ", ipmi::nvidia::netFnOemNV),
        entry("Cmd:[%02Xh]", ipmi::nvidia::misc::cmdGetDiagSystemConfig));

    ipmi::registerHandler(ipmi::prioOemBase, ipmi::nvidia::netFnOemNV,
                          ipmi::nvidia::misc::cmdGetDiagSystemConfig,
                          ipmi::Privilege::Admin, ipmi::getDiagSystemConfig);
    ...
    log<level::NOTICE>(
        "Registering ", entry("NetFn:[%02Xh], ", ipmi::nvidia::netFnOemNV),
        entry("Cmd:[%02Xh]", ipmi::nvidia::misc::cmdSetDiagResult));

    ipmi::registerHandler(ipmi::prioOemBase, ipmi::nvidia::netFnOemNV,
                          ipmi::nvidia::misc::cmdSetDiagResult,
                          ipmi::Privilege::Admin, ipmi::setDiagResult);

    log<level::NOTICE>(
        "Registering ", entry("NetFn:[%02Xh], ", ipmi::nvidia::netFnOemNV),
        entry("Cmd:[%02Xh]", ipmi::nvidia::misc::cmdGetDiagResult));

    ipmi::registerHandler(ipmi::prioOemBase, ipmi::nvidia::netFnOemNV,
                          ipmi::nvidia::misc::cmdGetDiagResult,
                          ipmi::Privilege::Admin, ipmi::getDiagResult);

    log<level::NOTICE>(
        "Registering ", entry("NetFn:[%02Xh], ", ipmi::nvidia::netFnOemNV),
        entry("Cmd:[%02Xh]", ipmi::nvidia::misc::cmdSetDiagFlowCtrl));

    ipmi::registerHandler(ipmi::prioOemBase, ipmi::nvidia::netFnOemNV,
                          ipmi::nvidia::misc::cmdSetDiagFlowCtrl,
                          ipmi::Privilege::Admin, ipmi::setDiagFlowCtrl);
    log<level::NOTICE>(
        "Registering ", entry("NetFn:[%02Xh], ", ipmi::nvidia::netFnOemNV),
        entry("Cmd:[%02Xh]", ipmi::nvidia::misc::cmdGetDiagFlowCtrl));

    ipmi::registerHandler(ipmi::prioOemBase, ipmi::nvidia::netFnOemNV,
                          ipmi::nvidia::misc::cmdGetDiagFlowCtrl,
                          ipmi::Privilege::Admin, ipmi::getDiagFlowCtrl);
#endif
    return;
}
