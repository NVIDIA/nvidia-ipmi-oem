...
static bool getSELLogFiles(std::vector<std::filesystem::path>& selLogFiles)
{
    ...
    for (const std::filesystem::directory_entry& dirEnt :
         std::filesystem::directory_iterator(nvidia_oem::ipmi::sel::selLogDir))
    {
        std::string filename = dirEnt.path().filename();
        if (boost::starts_with(filename, nvidia_oem::ipmi::sel::selLogFilename))
        {
            // If we find an ipmi_sel log file, save the path
            selLogFiles.emplace_back(nvidia_oem::ipmi::sel::selLogDir /
                                     filename);
        }
    }
...
using systemEventType = std::tuple<
    ...
    std::array<uint8_t, nvidia_oem::ipmi::sel::systemEventSize>>; // Event Data
using oemTsEventType = std::tuple<
    uint32_t,                                                   // Timestamp
    std::array<uint8_t, nvidia_oem::ipmi::sel::oemTsEventSize>>; // Event Data
using oemEventType =
    std::array<uint8_t, nvidia_oem::ipmi::sel::oemEventSize>; // Event Data
                           ...
                           uint8_t offset, uint8_t size)
{
    ...
    if (recordType == nvidia_oem::ipmi::sel::systemEvent)
    {
        // Get the timestamp
        std::tm timeStruct = {};
        ...
        uint8_t evmRev = nvidia_oem::ipmi::sel::eventMsgRev;
        ...
        std::array<uint8_t, nvidia_oem::ipmi::sel::systemEventSize> eventData{};
    ...
    else if (recordType >= nvidia_oem::ipmi::sel::oemTsEventFirst &&
             recordType <= nvidia_oem::ipmi::sel::oemTsEventLast)
    {
        // Get the timestamp
        std::tm timeStruct = {};
        std::istringstream entryStream(entryTimestamp);
        ...
        std::array<uint8_t, nvidia_oem::ipmi::sel::oemTsEventSize> eventData{};
        std::copy_n(eventDataBytes.begin(),
                    std::min(eventDataBytes.size(), eventData.size()),
                    eventData.begin());
    ...
    else if (recordType >= nvidia_oem::ipmi::sel::oemEventFirst)
    {
        // Only keep the bytes that fit in the record
        std::array<uint8_t, nvidia_oem::ipmi::sel::oemEventSize> eventData{};
        std::copy_n(eventDataBytes.begin(),
                    std::min(eventDataBytes.size(), eventData.size()),
                    eventData.begin());
                                           ...
                                           uint8_t eraseOperation)
{
    ...
    // Save the erase time
    nvidia_oem::ipmi::sel::erase_time::save();
